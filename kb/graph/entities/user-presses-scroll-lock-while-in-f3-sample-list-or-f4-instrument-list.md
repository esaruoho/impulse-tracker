...
- Scenario (L105): Scroll Lock inside the Pattern Editor still just toggles Follow Mode
- Scenario (L115): Scroll Lock in the Sample List opens the Pattern Editor with Follow Mode on
- Scenario (L129): Scroll Lock in the Instrument List does the same
- Scenario (L146): Ctrl-F in the Sample List (F3) or Instrument List (F4)
- Scenario (L155): Ctrl-F INSIDE the Pattern Editor (F2) toggles Follow Mode, not the config dialog
- Scenario (L173): Ctrl-F on the Order List (F11) or Song Variables (F12) enters the editor
- Scenario (L182): Follow Mode is forced ON, never toggled off, from the lists
- Scenario (L193): The handler hands Glbl_F2 the dispatcher's own DS (no segment damage)
- Scenario (L206): (not built) Scroll Lock / Ctrl-F from other screens (Order list F11, Song vars F12)
